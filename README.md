...
`arch` - Contains architecture specific implementations of the traits in `hal` crate.

`kernel` - The crate houses the code for interrupt handling, syscalls, context switching and scheduling.

`sync` - Contains architecture independent code for `Mutex` (xv6-style spinlocks), `RwLock`.

`userspace` - Contains two sample userspace programs to illustrate the working of context switching, trap handling, working of the stack and the heap. The directory also contains the code for the C Runtime and C syscall wrappers.
